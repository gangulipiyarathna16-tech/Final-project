...
SCRIPT_MAP = {
    ...
    "cuckoo"    : ("python3",     str(SCRIPTS_DIR / "cuckoo_client.py")),
}
...
def db_save_result(tool_id, target, out_rows, threat):
    """Parse tool["out"] rows and write one record to the correct scan table."""
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    def _find(prefix):
        """Return the text after ':' on the first row whose text contains prefix."""
        for _, text in out_rows:
    ...
    def _verdict_line():
        """Return the text of the first 'ok' or 'err' row (the VERDICT line)."""
        for kind, text in out_rows:
            if kind in ("ok", "err"):
                return text.strip()
        # fall back to last row
        return out_rows[-1][1].strip() if out_rows else ""

    try:
        c = sqlite3.connect(DB)
        if tool_id in ("malware", "ai", "cuckoo"):
            file_name  = target or _find("scan path") or _find("sample") or "unknown"
            confidence = _find("confidence") or _find("signatures") or ("HIGH" if threat else "LOW")
            scan_type  = {"malware": "Signature", "ai": "AI/ML", "cuckoo": "Sandbox"}.get(tool_id, tool_id)
            c.execute(
                "INSERT INTO malware_scan_logs(file_name,result,confidence,scan_type,timestamp)"
                " VALUES(?,?,?,?,?)",
                (file_name, _verdict_line(), confidence, scan_type, ts))

        elif tool_id == "backdoor":
            c.execute(
                "INSERT INTO backdoor_scans(scan_type,verdict,risk_score,target,timestamp)"
                " VALUES(?,?,?,?,?)",
                ("Backdoor Scan", _verdict_line(),
                 "HIGH" if threat else "LOW", target or "system", ts))

        elif tool_id in ("url", "domain"):
            domain     = target or _find("domain") or _find("target url") or "unknown"
            registrar  = _find("registrar") or "N/A"
            risk       = "HIGH" if threat else "LOW"
            c.execute(
                "INSERT INTO domain_logs(domain,result,risk_score,registrar,timestamp)"
                " VALUES(?,?,?,?,?)",
                (domain, _verdict_line(), risk, registrar, ts))

        elif tool_id == "port":
            open_ports = ", ".join(
                text.strip() for _, text in out_rows
                if "open" in text.lower() and "verdict" not in text.lower()
            ) or "none"
            c.execute(
                "INSERT INTO port_scans(target,result,risk_score,open_ports,timestamp)"
                " VALUES(?,?,?,?,?)",
                (target or "unknown", _verdict_line(),
                 "HIGH" if threat else "LOW", open_ports, ts))

        elif tool_id == "net":
            # Network scanner has no dedicated tab; skip DB write
            pass

        elif tool_id == "usb":
            usb_name      = _find("device") or "USB Device"
            files_scanned = _find("files scanned") or "N/A"
            c.execute(
                "INSERT INTO usb_scans(usb_name,result,files_scanned,timestamp)"
                " VALUES(?,?,?,?)",
                (usb_name, _verdict_line(), files_scanned, ts))

        elif tool_id == "vuln":
            cves = [text.strip() for _, text in out_rows
                    if re.search(r'CVE-\d{4}-\d+', text, re.I)]
            severity = "CRITICAL" if threat else "LOW"
            c.execute(
                "INSERT INTO vuln_findings(target,cve,severity,description,timestamp)"
                " VALUES(?,?,?,?,?)",
                (target or "unknown",
                 cves[0] if cves else "N/A",
                 severity, _verdict_line(), ts))

        c.commit(); c.close()
    except Exception as e:
        print(f"[db_save_result] {e}")
...
class OutputPanel(tk.Toplevel):
    ...
    def __init__(self, parent, tool_name, done_q=None):
        super().__init__(parent)
        ...
        self._proc   = None
        self._done_q = done_q   # queue.Queue — _stream puts threat bool when done
        self._threat = False
        self._build()
        self._poll()
    ...
    def _stream(self):
        """Read stdout line-by-line, detect threats, push to display queue."""
        threat_words = [
            "malicious", "threat detected", "backdoor",
            "reverse shell", "critical", "verdict",
        ]
        for line in iter(self._proc.stdout.readline, ""):
            if any(w in line.lower() for w in threat_words):
                self._threat = True
            self._queue.put(line)
        self._proc.stdout.close()
        self._proc.wait()
        self._queue.put(None)           # tell _poll we are done
        if self._done_q is not None:    # signal _work thread
            self._done_q.put(self._threat)
...
class ToolCard(tk.Frame):
    def __init__(self, parent, tool, user,
        ...
        super().__init__(parent, bg=CARD,
                         highlightthickness=1,
                         highlightbackground=BORDER,
                         cursor=("arrow" if (auto_run and tool["id"] not in NEEDS_TARGET)
                                 else "hand2"), **kw)
        self.tool      = tool
        self.user      = user
        ...
        if auto_run:
            # Schedule auto-run; NEEDS_TARGET tools will wait for a click instead
            self.after(auto_delay, self._start_auto)

        # Add click bindings for manual tools and for auto_run cards that
        # require a target (malware, ai, vuln) so users can enter one
        if not auto_run or tool["id"] in NEEDS_TARGET:
    ...
    def _start_auto(self):
        """Auto-run only tools that need no target (backdoor, cuckoo).
        Tools in NEEDS_TARGET wait for the user to click the card."""
        if self._running:
            return
        if self.tool["id"] in NEEDS_TARGET:
            self._svar.set("Click card to enter target")
            self._slbl.config(fg=T3)
            return
        self._start(None)
    ...
    def _work(self, target):
        ...
        # ── Build stdin string (text=True subprocess, no encode/decode) ──
        stdin_input = None
        if tid == "url":
            stdin_input = target + "\n"
        elif tid == "domain":
            ...
            stdin_input = "1\n3\n"
        elif tid in ("malware", "ai"):
            stdin_input = "1\n" + target + "\n4\n"
        elif tid == "vuln":
            stdin_input = "1\n" + target + "\n2\n"
        # backdoor and cuckoo need no stdin (stdin_input stays None)
        ...
        try:
            ...
            # Write stdin FIRST so Bash read commands get input immediately
            if stdin_input:
                try:
            ...
            # OutputPanel owns stdout reading and signals result_q when done
            self.after(0, lambda: self._open_panel(tid, proc, result_q))

            # Block until panel signals done (timeout = 130 s)
            timed_out = False
            try:
                threat = result_q.get(timeout=130)
            except queue.Empty:
                threat    = False
                timed_out = True
                proc.kill()
            ...
            step_thread.join()
            self.after(0, lambda: self._finish(
                False, error=f"Launch error: {e}"))
            return

        step_thread.join()
        if timed_out:
            self.after(0, lambda: self._finish(
                False, error="Scan timed out after 130 seconds."))
        else:
            # Save result to DB before updating UI
            db_save_result(tid, target, self.tool["out"], threat)
            self.after(0, lambda h=threat: self._finish(h))

    def _open_panel(self, tid, proc, done_q):
        """Open the live output panel; panel owns stdout and signals done_q."""
        panel = OutputPanel(self.winfo_toplevel(), self.tool["name"], done_q)
...
class ResultsViewer(tk.Frame):
    ...
    def __init__(self, parent, user, **kw):
        ...
        self._active    = 0
        self._build()
        # _refresh_loop calls _load() immediately, then every 10 seconds
        self._refresh_loop()
    ...
    def _load_all(self, cur):
        ...
        queries = [
            ...
            ("Vulnerability Scanner",
             "SELECT id, target, verdict, risk_score, timestamp FROM vuln_findings ORDER BY id DESC LIMIT 20"),
        ]
...
class Dashboard(tk.Frame):
    ...
    def _build(self):
        ...
        # ── Tab bar ───────────────────────────
        tab_bar = tk.Frame(self, bg="#0a1628")
        tab_bar.pack(fill="x")
        ...
        for name in ["Tools", "Results"]:
            btn = tk.Button(
                tab_bar, text=f"  {name}  ",
                bg=BLUE if name == "Tools" else "#0a1628",
                fg="#ffffff" if name == "Tools" else "#5a7898",
                font=fnt(11, name == "Tools"),
                relief="flat", cursor="hand2",
                padx=10, pady=8,
                activebackground=BLUE,
                activeforeground="#ffffff",
        ...
        self._tab_frames["Tools"] = tools_frame
        ...
        tk.Frame(tools_frame, bg=BORDER,
        ...
        self._tab_frames["Results"] = results_frame
        if can(self._user, "view_results"):
            self._results_viewer = ResultsViewer(results_frame, self._user)
            self._results_viewer.pack(fill="both", expand=True)
        else:
            self._results_viewer = None
            self._locked(results_frame, "Scan Results", BLUE).pack(
                fill="both", expand=True)
    ...
    def _switch_tab(self, name):
        ...
        for n, btn in self._tab_btns.items():
            btn.configure(
                bg=BLUE if n == name else "#0a1628",
                fg="#ffffff" if n == name else "#5a7898",
                font=fnt(11, n == name))
